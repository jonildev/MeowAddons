...
slayerbossdisplay
  ...
  .registersub(
    ...
    () => {
      const hpOffset =
        Renderer.getStringWidth(`&c☠ &b${bossName}`) -
        Renderer.getStringWidth(hp);
...
register(
  Java.type("net.minecraftforge.event.entity.EntityJoinWorldEvent"),
  (entity) => {
    if (settings().slayerbossdisplay || settings().slayerkilltimer) {
      scheduleTask(() => {
        const name = ChatLib.removeFormatting(entity.entity.func_70005_c_());
        if (
          name.includes("Spawned by") &&
          name.split("by: ")[1] === Player.getName()
        ) {
...
register("entityDeath", (entity) => {
  ...
  if (bossIDdeath === bossID) {
    ...
    if (settings().slayerkilltimer) {
      ChatLib.chat(
        `&e[MeowAddons] &fYou killed your boss in &b${(
          timeTaken / 1000
        ).toFixed(2)}s&f.`
      );
    }
...
register("chat", () => resetBossTracker()).setCriteria(
  /&r  &r&c&lSLAYER QUEST FAILED!&r/
);
